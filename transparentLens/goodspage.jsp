...
<body>
	<%
	String idTransparentLens = request.getParameter("idTransparentLens");
	...
	try {
	    ...
	    if(con.isClosed()){
			...
			String sql = "SELECT * FROM `transparentlens` WHERE `idTransparentLens`=?";
			PreparedStatement pstmt = null;
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1,idTransparentLens);
  ...
  <table class="infor">
    <tr>
      <td>
        <img class="pp" src="image/<%=dataset.getString("idTransparentLens")%>.png?time=<%=System.currentTimeMillis()%>" width="350px" height="400px">
      </td>
      <td>
        <ul>
          <li><h3>商品名稱:<%=dataset.getString("transparentLensName")%></h3></li>
          <li><h3>規格說明:<br>  
		  <br>含水量:<%=dataset.getString("transparentLensWaterContent")%>%<br>  
		  <br>基弧:<%=dataset.getString("transparentLensBaseCurve")%>mm<br>  
		  <br>鏡片直徑:<%=dataset.getString("transparentLensDiameter")%>mm<br>  
		  <br>著色直徑:<%=dataset.getString("transparentLensGraphicDiameter")%>mm<br></h3></li>
        </ul>
	<%}
		}
